...
import { useBatchReport } from '@/hooks/warehouse/useBatchReport';
import type {
  ReportInvoiceLine,
  ReportProductionLine,
} from '@/types/warehouse/warehouse.types';

export function BatchReportPage(): ReactNode {
  const { t } = useTranslation(['common', 'documents', 'tables']);
  const { id } = useParams<{ id: string }>();
  const batchId = Number(id);
  const { data, loading, error } = useBatchReport(batchId);

  const invoiceColumns: MRT_ColumnDef<ReportInvoiceLine>[] =
    useReportInvoiceColumns({ linkProduct: true, linkBatch: false });
  ...
  return (
    <Stack gap='lg' p='md'>
      <Title order={2}>
        {data.batch.product.name}, {t('tables:columns.batch').toLowerCase()}{' '}
        {data.batch.name} ({t('documents:documents.batchReport')})
      </Title>
