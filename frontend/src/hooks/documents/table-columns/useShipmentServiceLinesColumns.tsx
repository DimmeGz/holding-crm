...
import type { ShipmentServiceLine } from '@/types/documents/shipments.types';
import { CommonConstants } from '@/constants/common.constants';
import { useLibsStore } from '@/stores/useLibsStore';

export function useShipmentServiceLinesColumns(
  currency: string,
): MRT_ColumnDef<ShipmentServiceLine>[] {
    ...
    () => [
      {
        ...
        id: 'serviceName',
        accessorFn: (row: ShipmentServiceLine): string =>
          getServiceName(row.serviceId),
        size: 200,
        Cell: ({ cell }: { cell: MRT_Cell<ShipmentServiceLine> }) =>
      ...
      },
      {
        ...
        id: 'amount',
        accessorFn: (row: ShipmentServiceLine): number => row.price * row.qty,
        size: 140,
        Cell: ({ cell }: { cell: MRT_Cell<ShipmentServiceLine> }) =>
          `${cell.getValue<number>()} ${currency}`,
      },
